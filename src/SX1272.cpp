...
/*
 Function: It stores the last ACK received packet at the 
 address provided by the caller after verificaiton of the source address
Returns: '0' on success, '1' otherwise
 */ 
int8_t SX1272::getAckPacket(uint8_t src, uint8_t *payload, uint8_t *size) {
    uint8_t ret=1;
    //Serial.println(F("Starting 'getACKpacket'"));
   ...
   */
   uint8_t SX1272::getACK(uint16_t wait)
   {
    ...
    if( a_received )
    {
        ...
        for (uint32_t i=0;i<ACK.length;i++) {
            Serial.print(" ");
            ACK.data[i] = readRegister(REG_FIFO);
            // Serial.print(ACK.data[i]);
        }
        // Serial.println(); 
        #endif
